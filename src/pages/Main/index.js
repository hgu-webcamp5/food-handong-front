...
function Star(props) {
  return <SvgIcon sx={{ fontSize: 15, color: props.color }} component={StarIcon} inheritViewBox />;
}
function Heart(props) {
  return (
    <SvgIcon
      sx={{ fontSize: 15, ml: 1, color: props.color }}
  ...
  useEffect(() => {
    const categoryCountFunc = async () => {
      ...
      categories.map((category, index) => {
        categoryCount[index] = data.filter(
      ...
      <div>
        ...
        >
          ...
          <Grid item xs={12} sm={8} md={10}>
            ...
            >
              {(categoryFilter !== '전체보기'
                ? restaurants.filter((restaurant) => restaurant.category.name === categoryFilter)
                : restaurants
                ...
                >
                  ...
                  <Box component="div" id="block-firstLine" sx={{ display: 'block' }}>
                    <Typography variant="body1" sx={{ float: 'left', fontWeight: 'bold' }}>
                      {restaurant.name}
                    </Typography>{' '}
                    <Typography variant="body2" sx={{ float: 'right', fontWeight: 'bold' }}>
                      {restaurant.category.name}
                    </Typography>
                  </Box>
                  <Box component="div" id="block-secondLine" sx={{ display: 'block', mt: 3.5 }}>
                    <Typography variant="body2" sx={{ mt: 1, float: 'left', fontWeight: 'bold' }}>
                      {restaurant.dong}
                    </Typography>{' '}
                    {restaurant.recommendation ? (
                      <Typography
                        variant="body2"
                        sx={{ float: 'right', color: 'error.main', fontWeight: 'bold' }}
                      >
                        추천!
                      </Typography>
                    ) : null}
                  ...
                  <Box id="block-lastLine" sx={{}}>
                    <Typography variant="body2" sx={{ float: 'right' }}>
                      <Comment color={'gray'} /> {restaurant.comment}
                    </Typography>
                    <Typography variant="body2" sx={{ float: 'right' }}>
                      <Heart color={'error.main'} /> {restaurant.heart}
                    </Typography>
                    <Typography variant="body2" sx={{ float: 'right' }}>
                      <Star color={'orange'} /> {restaurant.rate}
                    </Typography>
                  </Box>
                </Item>
              ))}
